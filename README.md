...
Create an interactive, browsable and editable map of an existing Moodle
course from the processed outputs of Moodle Course Audit.

Moodle Course Content Mapper preserves the current Moodle section and
activity structure and connects activities to extracted course resources.
Its **primary output is an interactive HTML course map** for exploring the
existing course, with navigation, filtering and clickable links to mapped
resources.

It also generates an **editable Microsoft Word version** of the map for
human-led course review and redesign. In Word, headings, activities, text
and linked resource references can be moved, renamed, annotated and
reorganised to sketch a proposed new structure while retaining links back
...
## Upstream dependency: Moodle Course Audit

Moodle Course Content Mapper does **not** currently process Moodle
`.mbz` backup files directly.

It consumes the processed course output generated by **Moodle Course
Audit**. Moodle Course Audit must be run with file/resource extraction
enabled so that the processed course contains both:
...
-   `audit/` --- Moodle sections, activities, placement information and
    structural metadata.
-   `extracted_files/` --- the actual course files used to create
